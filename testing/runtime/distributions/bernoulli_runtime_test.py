...
import mxnet as mx
import numpy as np
from mxfusion.runtime.distributions import BernoulliRuntime
...
class TestBernoulliRuntimeDistribution(object):
    ...
    def test_log_pdf(self, dtype, prob, rv):
        ...
        log_pdf_np = bernoulli.logpmf(rv, p=prob)
        ...
        bernoulli_dist = BernoulliRuntime(prob_true=prob_mx)
        log_pdf_rt = bernoulli_dist.log_pdf(rv_mx)
    ...
    def test_draw_samples(self):
        ...
        bernoulli_dist = BernoulliRuntime(prob_true=prob_mx)
        samples = bernoulli_dist.draw_samples(num_samples)
        rtol, atol = 1e-1, 1e-1
        assert np.allclose(samples.asnumpy().mean(0), bernoulli_dist.mean.asnumpy(), rtol=rtol, atol=atol)
        assert np.allclose(samples.asnumpy().var(0), bernoulli_dist.variance.asnumpy(), rtol=rtol, atol=atol)
