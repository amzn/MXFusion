...
class TestConditionalGaussianProcessDistribution(object):
    ...
    def test_log_pdf_w_mean(self, dtype, X, X_isSamples, X_cond, X_cond_isSamples, Y_cond, Y_cond_isSamples, rbf_lengthscale, rbf_lengthscale_isSamples, rbf_variance, rbf_variance_isSamples,
                        rv, rv_isSamples, num_samples):
        ...
        gp = ConditionalGaussianProcess.define_variable(X=X_var, X_cond=X_cond_var, Y_cond=Y_cond_var, mean=mean_var, mean_cond=mean_cond_var, kernel=rbf, shape=rv_shape, dtype=dtype).factor

        variables = {gp.X.uuid: X_mx, gp.X_cond.uuid: X_cond_mx, gp.Y_cond.uuid: Y_cond_mx, gp.rbf_lengthscale.uuid: rbf_lengthscale_mx, gp.rbf_variance.uuid: rbf_variance_mx, gp.random_variable.uuid: rv_mx, gp.mean.uuid: mean_mx, gp.mean_cond.uuid: mean_cond_mx}
        log_pdf_rt = gp.log_pdf(F=mx.nd, variables=variables).asnumpy()
        ...
        for i in range(num_samples):
            ...
            X_cond_i = X_cond[i] if X_cond_isSamples else X_cond
            Y_cond_i = Y_cond[i] if Y_cond_isSamples else Y_cond
            Y_cond_i = Y_cond_i - mean_cond_np[i] if X_cond_isSamples else Y_cond_i - mean_cond_np[0]
            lengthscale_i = rbf_lengthscale[i] if rbf_lengthscale_isSamples else rbf_lengthscale
            variance_i = rbf_variance[i] if rbf_variance_isSamples else rbf_variance
            rv_i = rv[i] if rv_isSamples else rv
    ...
    def test_draw_samples_w_mean(self, dtype, X, X_isSamples, X_cond, X_cond_isSamples, Y_cond, Y_cond_isSamples, rbf_lengthscale, rbf_lengthscale_isSamples, rbf_variance, rbf_variance_isSamples,
                        rv_shape, num_samples):
        net = nn.HybridSequential(prefix='nn_')
        with net.name_scope():
        ...
        mean_var = mean_func(X_var)
        mean_cond_var = mean_func(X_cond_var)
        gp = ConditionalGaussianProcess.define_variable(X=X_var, X_cond=X_cond_var, Y_cond=Y_cond_var, mean=mean_var, mean_cond=mean_cond_var, kernel=rbf, shape=rv_shape, dtype=dtype, rand_gen=rand_gen).factor

        variables = {gp.X.uuid: X_mx, gp.X_cond.uuid: X_cond_mx, gp.Y_cond.uuid: Y_cond_mx, gp.rbf_lengthscale.uuid: rbf_lengthscale_mx, gp.rbf_variance.uuid: rbf_variance_mx, gp.mean.uuid: mean_mx, gp.mean_cond.uuid: mean_cond_mx}
        samples_rt = gp.draw_samples(F=mx.nd, variables=variables, num_samples=num_samples).asnumpy()
    ...
    def test_mean_argument(self):
        ...
        with pytest.raises(ModelSpecificationError):
            ...
            net = nn.HybridSequential(prefix='nn_')
            with net.name_scope():
                net.add(nn.Dense(1, flatten=False, activation="tanh",
                                 in_units=2, dtype=dtype))
            net.initialize(mx.init.Xavier(magnitude=3))
            ...
            mean_var = mean_func(X_var)
            mean_cond_var = mean_func(X_cond_var)
            gp = ConditionalGaussianProcess.define_variable(X=X_var, X_cond=X_cond_var, Y_cond=Y_cond_var, mean=mean_var, kernel=rbf, shape=(5, 1), dtype=dtype)
        ...
        with pytest.raises(ModelSpecificationError):
            ...
            mean_func = MXFusionGluonFunction(net, num_outputs=1,
                                              broadcastable=True)
            mean_var = mean_func(X_var)
            gp = ConditionalGaussianProcess(X=X_var, X_cond=X_cond_var, Y_cond=Y_cond_var, mean=mean_var, kernel=rbf, dtype=dtype)
        ...
        with pytest.raises(ModelSpecificationError):
            ...
            mean_var = mean_func(X_var)
            mean_cond_var = mean_func(X_cond_var)
            gp = ConditionalGaussianProcess.define_variable(X=X_var, X_cond=X_cond_var, Y_cond=Y_cond_var, mean_cond=mean_cond_var, kernel=rbf, shape=(5, 1), dtype=dtype)
        ...
        with pytest.raises(ModelSpecificationError):
            ...
            mean_func = MXFusionGluonFunction(net, num_outputs=1,
                                              broadcastable=True)
            mean_cond_var = mean_func(X_cond_var)
            gp = ConditionalGaussianProcess(X=X_var, X_cond=X_cond_var, Y_cond=Y_cond_var, mean_cond=mean_cond_var, kernel=rbf, dtype=dtype)
