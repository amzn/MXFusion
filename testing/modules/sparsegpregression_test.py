...
class TestSparseGPRegressionModule(object):
    ...
    def test_log_pdf_w_mean(self):
        ...
        # GPy log-likelihood
        m_gpy = GPy.models.SparseGPRegression(X=X, Y=Y-mean, Z=Z, kernel=GPy.kern.RBF(3, ARD=True, lengthscale=lengthscale, variance=variance), num_inducing=3)
        m_gpy.likelihood.variance = noise_var
        l_gpy = m_gpy.log_likelihood()
    ...
    def test_sampling_prediction(self):
        ...
        gp.sgp_predict.noise_free = False
    ...
    def test_sampling_prediction_w_mean(self):
        ...
        gp.attach_prediction_algorithms(
            ...
            algorithm=SparseGPRegressionSamplingPrediction(
                gp._module_graph, gp._extra_graphs[0], [gp._module_graph.X]),
            alg_name='sgp_predict')
        gp.sgp_predict.diagonal_variance = True
        gp.sgp_predict.noise_free = False
        gp.sgp_predict.jitter = 1e-6
    ...
    def test_module_clone_prediction(self):
        ...
        m = self.gen_mxfusion_model(dtype, D, Z, noise_var, lengthscale, variance)
        ...
        infr2_clone.inference_algorithm.model.Y.factor.sgp_predict.diagonal_variance = False
        infr2_clone.inference_algorithm.model.Y.factor.sgp_predict.noise_free = False
    ...
    def test_module_clone_samples(self):
        D, X, Y, Z, noise_var, lengthscale, variance = self.gen_data()
        dtype = 'float64'
        ...
        mx.random.seed(123)
        res = infr2_clone.run(X=mx.nd.array(X, dtype=dtype))[0]
        samples_clone = res.asnumpy()
        assert np.allclose(samples, samples_clone)
