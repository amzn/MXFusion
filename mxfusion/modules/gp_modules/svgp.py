...
from ...inference.variational import StochasticVariationalInference, VariationalSamplingAlgorithm
from ...util.customop import make_diagonal


class SVGP_log_pdf(StochasticVariationalInference):
    def __init__(self, model, posterior, observed, num_samples=1, jitter=0.):
        super(SVGP_log_pdf, self).__init__(
            num_samples=num_samples, model=model, posterior=posterior,
            observed=observed)
        self.jitter = jitter
    ...
    def compute(self, F, variables):
        ...
        likelihood = self.model.Y.factor
        ...
        # for sampler
        f_mean = F.linalg.gemm2(LinvKuf, Linvmu, True, False)
        tmp = F.linalg.gemm2(LinvSLinvT, LinvKuf)
        f_var = F.expand_dims(Kff_diag - F.sum(F.square(LinvKuf), axis=-2) + F.sum(tmp*LinvKuf, axis=-2), axis=-1)

        f_samples = F.random.normal(shape=(self.num_samples,) + f_mean.shape[1:], dtype=f_mean.dtype) * F.sqrt(f_var) + f_mean
        variables[self.model.F.uuid] = f_samples

        logL = likelihood.compute_log_prob(F, variables)
...
class SVGP(Module):
    ...
    def __init__(self, X, kernel, likelihood_dist, likelihood_input, F_dim, inducing_inputs=None, num_samples=1,
        ...
        self.likelihood_input = likelihood_input
    ...
    def _build_module_graphs(self, output_variables):
        ...
        graph.U = GaussianProcess.define_variable(
            ...
            shape=(graph.inducing_inputs.shape[0], self.F_dim),
        ...
        graph.F = ConditionalGaussianProcess.define_variable(
            ...
            kernel=self.kernel, shape=Y.shape[:-1] + (self.F_dim,), mean_func=self.mean_func,
            rand_gen=self.rand_gen, dtype=self.dtype, ctx=self.ctx)
        graph.Y = Y.replicate_self()
        self.likelihood_dist.set_input_variable(self.likelihood_input, graph.F)
        graph.Y.set_prior(self.likelihood_dist)
        graph.mean_func = self.mean_func
        graph.kernel = graph.U.factor.kernel
        post = Posterior(graph)
        post.qU_cov_diag = Variable(shape=(self.F_dim, M), transformation=PositiveTransformation())
        post.qU_cov_W = Variable(shape=(self.F_dim, M, M))
        post.qU_mean = Variable(shape=(M, self.F_dim))
        return graph, [post]
    ...
    def _attach_default_inference_algorithms(self):
        ...
        self.attach_log_prob_algorithms(
            ...
            algorithm=SVGP_log_pdf(self._module_graph, self._extra_graphs[0],
                                   observed, num_samples=self.num_samples))
